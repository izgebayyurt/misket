...
/// Substring or regular-expression search across every text document's
/// content.
///
/// With `regex` false, matching folds ASCII case only (non-ASCII characters
/// must match exactly); see `fold_ascii`. With `regex` true, `query` is
/// compiled as a standard (case-sensitive; use an inline `(?i)` for
/// case-insensitive) regular expression via the `regex` crate, and an
/// invalid pattern is a `Validation` error. Either way, matches are anchored
/// on Unicode code point boundaries, an empty or whitespace-only query
/// returns no hits, and the total is capped at `limit` (which is itself
/// capped at [`MAX_HITS`]) across the whole project. Results are ordered by
/// document (`sort_order`, `created_at`) then by position within the
/// document.
        ...
        for (document_id, document_name, doc_text) in docs {
            ...
            for m in re.find_iter(&doc_text) {
                ...
                let (context_before, context_after) =
                    context(&doc_text, total_len, start_pos, end_pos);
                hits.push(SearchHit {
...
mod tests {
    ...
    fn regex_mode_matches_a_pattern_case_sensitively_by_default() {
        ...
        let hits = search_project(&p.conn, r"[Cc]at\d+", 50, true).unwrap();
        assert_eq!(hits.len(), 2);
        ...
        let hits = search_project(&p.conn, r"cat\d+", 50, true).unwrap();
        assert_eq!(hits.len(), 1);
        let hits = search_project(&p.conn, r"(?i)cat\d+", 50, true).unwrap();
        assert_eq!(hits.len(), 2);
    }
    ...
    fn regex_mode_anchors_matches_on_code_points_with_multibyte_text() {
        ...
        let hits = search_project(&p.conn, r"w\w+d", 50, true).unwrap();
        assert_eq!(hits.len(), 1);
    ...
    fn regex_mode_rejects_an_invalid_pattern() {
        ...
        assert!(matches!(
            search_project(&p.conn, "(unclosed", 50, true),
    ...
    fn regex_mode_handles_empty_matches_without_looping() {
        ...
        let hits = search_project(&p.conn, "x*", 50, true).unwrap();
    ...
    fn limit_is_capped_at_max_hits() {
        ...
        let hits = search_project(&p.conn, "a", 50_000, false).unwrap();
