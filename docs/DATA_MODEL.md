...
| `transcript_anchors` | `(document_id, pos, ms)`, `WITHOUT ROWID`: one point where a transcript and its recording are known to meet — a code point offset and a millisecond. Everything between two of them is interpolated ([Transcript alignment](#transcript-alignment))                                                                                                                                                                                                                                                                                                         |
...
A text document can point at the recording it transcribes:
`documents.linked_media_id` (schema 15), set and cleared by
`db::align::link` and undoable as `document.linked`. The link lives on the
**text** side, so deleting the recording leaves the transcript intact — the
foreign key is `ON DELETE SET NULL` — and a media document's summary carries
`transcriptId`, the other end, found with one sub-query per row.

`ON DELETE SET NULL` would lose the link when a recording is deleted and
undone, so `DocumentSnapshot` carries `linked_by`: the text documents that
pointed at this one. `documents::restore` sets their links again, skipping
any that have been deleted since. The same snapshot carries `anchors`,
because `transcript_anchors` cascades with the document.

### The mapping

`transcript_anchors(document_id, pos, ms)` holds the points where the two are
known to meet; `text::align` turns a handful of them into a mapping over the
whole document:

- **Between two anchors**, linearly. Speech is not uniform, but over a cue or
  a turn it is close enough to put the playhead in the right sentence.
- **Before the first anchor**, the recording is assumed to start at 0 ms: the
  segment from `(0, 0)` to the first anchor is interpolated like any other.
  An anchor _at_ position 0 is allowed and wins, which is how a subtitle file
  whose first cue starts thirty seconds in says so.
- **After the last anchor**, at the document's average rate —
  `last.ms / last.pos` milliseconds per code point.

Both directions are total: with no anchors at all everything is at 0 ms. The
mapping is kept monotone — anchors are sorted by position and a time that
would run backwards is clamped up to its predecessor — so `ms_to_pos` has
exactly one answer. Across a flat stretch (two anchors at the same
millisecond, which a pause produces) it answers with the start of that
stretch, so the playhead highlights the passage about to be read.

`src/core/align.ts` mirrors `text::align` exactly. The backend owns the
mapping when it writes; the view needs the same answer several times a second
while the recording plays, and an IPC round trip per frame is not that. Both
are tested; the rules are written down once, here.

### Where anchors come from

| Source                          | What it does                                                                                                                                                        |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SRT/VTT import                  | one anchor per cue, at the cue's paragraph start; passed to `create_document` as `anchors` and written before the import is snapshotted, so undo and redo keep them |
| "Build anchors from timestamps" | every turn whose `time` group parses (`mm:ss`, `h:mm:ss`, `mm:ss.mmm`, in `[…]` or `(…)`), anchored on the spoken text rather than the label                        |
| "Align here" (`Alt`+`A`)        | the caret (or the selection's start) is heard at the playhead now                                                                                                   |

Each write records the document's **whole** anchor list on both sides
(`history::AnchorChange`), which is an exact inverse for setting one,
removing one and rebuilding all of them alike.

`db::align::code_recording_for_excerpt` is the one place both ends are
written at once: it maps a text excerpt's range through `pos_to_ms` and
applies the same codes to a `video_range` excerpt on the linked recording, in
one history group.
