...
export function QueryBuilder({
  ...
  const label = query ? describeQuery(query, codeName) : "Query";
    ...
    <>
      ...
      >
        ...
        <PopoverContent align="start" className="w-96 p-3">
          ...
          <p className="mt-2 text-xs text-fg-muted">{problem ?? describeQuery(draft, codeName)}</p>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="min-w-0 flex-1 truncate text-xs text-fg-muted">{OP_HELP[draft.op]}</span>
      </div>
      {draft.op === "near" ? (
        <div className="flex items-center gap-2">
          <select
          ...
          >
            <option value="paragraph">in the same paragraph</option>
            <option value="chars">within N characters</option>
          </select>
          {scope.kind === "chars" ? (
            <Input
              type="number"
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <label
        ...
        title="Count excerpts coded with any sub-code of this one too"
      >
        <input
          type="checkbox"
